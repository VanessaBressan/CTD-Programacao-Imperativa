...
const curso = {
    ...
    // Crie um método para o objeto curso que receba um aluno (como parâmetro) e retorne true se ele aprovou no curso ou 
    ...
    verificarAprovacao(aluno) {
        if (aluno.calcularMedia() >= this.notaAprovacao && aluno.faltas < this.faltasMaximas) {
            return true;
        } else if (aluno.faltas === this.faltasMaximas && aluno.calcularMedia()>(this.notaAprovacao+(this.notaAprovacao*0.1))) {
            return true;
        } else {
            return false;
        };
...
curso.adicionarAluno('Thomas', 0, [10, 10, 9, 9])
curso.listaEstudantes[0].faltas()
